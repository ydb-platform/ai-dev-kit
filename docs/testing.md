...
- **System prompt:** the installable skills' content embedded into one system message. `ydb-core` is loaded as `SKILL.md` plus its `references/balancing.md`, `references/session-lifecycle.md`, `references/embed/{go,python,cpp}.md`, and `rules/embed/go.md`. `ydb-table` is loaded as `SKILL.md` plus its `references/working-with-data.md`, `references/embed/java.md`, `references/embed/go.md`, `rules/embed/java.md`, and `rules/embed/go.md` — i.e. the full body of each skill, as if the agent had read every Load-Sources entry.
