...
### RULE-GO-12: Scan query used for a read the Query Service can serve

**Severity**: Medium

**What to look for**: `s.StreamExecuteScanQuery(ctx, ...)` on a `table.Session`, `options.WithExecuteScanQueryMode(...)` / `options.ExecuteScanQueryOption` in a helper signature, `options.ExecuteScanQueryRequestModeExec`. On the `database/sql` surface: `ydb.WithQueryMode(ctx, ydb.ScanQueryMode)`, `ydb.WithDefaultScanQueryOptions(...)` on the connector, or a `query_mode=scan` (legacy `go_query_mode=scan`) parameter in the DSN. Any of these in new code, or in code being modified, is the target.

**Problem**: scan queries are the deprecated Table Service analytics interface — upstream marks the functionality deprecated and directs new code at standard query execution. The reason developers reach for them is real: Table Service `s.Execute` caps results at 1000 rows and a scan stream has no such cap. But the Query Service removes that cap too, without what a scan costs: prepared statements are unsupported, so the query recompiles on every call; the SDK does not retry a scan automatically, so a transient failure surfaces raw to the caller and the surrounding `Do` buys nothing; query duration is capped at 10 minutes; sorting and other operations run entirely in memory and fail with out-of-resources on complex queries; joins support only the MapJoin strategy, so the right-hand table must fit in a few gigabytes; and point or small-range reads get no optimization. A scan competes for the same CPU, memory, disk and network as production traffic, so a heavy one can starve the whole database.

**Fix**: move the read to the Query Service — `db.Query().Do(ctx, func(ctx context.Context, s query.Session) error { res, err := s.Query(ctx, sql, query.WithParameters(ydb.ParamsBuilder()...)); ... }, query.WithIdempotent())`. It streams result sets with no row cap, so the reason for the scan disappears. On the `database/sql` surface, drop the scan mode and let the connector use the Query Service default. When the match is large enough that replaying it on a mid-stream failure is unacceptable, wrap the read in keyset pagination over the primary key so each batch is its own retry scope. For a genuinely analytical workload, a column-oriented table is the durable answer rather than a scan over a row-oriented one.

**Source**: <https://ydb.tech/docs/en/concepts/query_execution/scan_query>; `ydb-go-sdk/v3` `table/table.go` (`Session.StreamExecuteScanQuery`), `sql.go` (`ScanQueryMode`, `WithDefaultScanQueryOptions`), `dsn.go` (`query_mode` parsing).
