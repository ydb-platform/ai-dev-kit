...
- **`NYdb::NQuery::TQueryClient`** — Query Service (preferred): `StreamExecuteQuery`, fused `TTxControl`, `ReadCommittedRW`.
...
`RetryQuerySync(lambda, TRetryOperationSettings)` — the **lambda is the retry unit**. Second arg: `.Idempotent(true)` when replay-safe (reads, client-keyed UPSERT); omit for non-idempotent writes.
...
Single statement: `TTxControl::BeginTx(TTxSettings::SerializableRW()).CommitTx()` on the query call. Multi-step: first query `BeginTx()` without `CommitTx`, later `TTxControl::Tx(tx).CommitTx()`. Modes: [`../working-with-data.md`](../working-with-data.md).

## Retries

Use SDK retriers — not outer `for` on `RetryQuerySync` (RULE-CPP-04) and not `sleep_for` + bare `ExecuteQuery` (RULE-CPP-05). Status classes: always-retry (`ABORTED`, `UNAVAILABLE`, `BAD_SESSION`); conditional with `.Idempotent(true)` (`UNDETERMINED`, `TRANSPORT_UNAVAILABLE`); non-retryable (`PRECONDITION_FAILED`, `SCHEME_ERROR`). Tune via `TRetryOperationSettings`. Source: <https://ydb.tech/docs/en/reference/ydb-sdk/ydb-status-codes>.

## Large reads & streams

- **`ExecuteDataQuery`**: check `TResultSet::Truncated()` or paginate / use `StreamExecuteScanQuery` (RULE-CPP-01).
- **`StreamExecuteQuery`**: `ReadNext()` parts; retrier replay can re-emit rows — dedupe side effects (RULE-CPP-09; basic example comment in rules file).
